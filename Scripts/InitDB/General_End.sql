...
grant usage on schema app_public to rls_obeyer;
--grant all on schema app_public to rls_obeyer;

-- approach 1 (doesn't work, because the "default permissions" are only used for future tables that are made)
--alter default privileges in schema app_public grant select, insert, update, delete on tables to rls_obeyer;
-- approach 2 (works): loop through all tables, granting permissions
grant select, insert, update, delete on all tables in schema app_public to rls_obeyer;