...
CREATE OR REPLACE FUNCTION app_public.map_node_edits_refresh_targets_for_self() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
	IF (
		...
		UPDATE app_public."mapNodeEdits" SET "c_accessPolicyTargets" = array[
...
CREATE TRIGGER map_node_edits_refresh_targets_for_self AFTER INSERT OR UPDATE ON app_public."mapNodeEdits" FOR EACH ROW EXECUTE FUNCTION app_public.map_node_edits_refresh_targets_for_self();
...
CREATE OR REPLACE FUNCTION app_public.node_links_refresh_targets_for_self() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
	IF (
		...
		UPDATE app_public."nodeLinks" SET "c_accessPolicyTargets" = array[
...
CREATE TRIGGER node_links_refresh_targets_for_self AFTER INSERT OR UPDATE ON app_public."nodeLinks" FOR EACH ROW EXECUTE FUNCTION app_public.node_links_refresh_targets_for_self();
...
CREATE OR REPLACE FUNCTION app_public.node_phrasings_refresh_targets_for_self() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
	IF (
		...
		UPDATE app_public."nodePhrasings" SET "c_accessPolicyTargets" = array[
			(SELECT concat((SELECT "accessPolicy" FROM "nodes" WHERE id = NEW."node"), ':nodes'))
		] WHERE id = NEW.id;
	END IF;
END $$;
DROP TRIGGER IF EXISTS node_phrasings_refresh_targets_for_self on app_public."nodePhrasings";
CREATE TRIGGER node_phrasings_refresh_targets_for_self AFTER INSERT OR UPDATE ON app_public."nodePhrasings" FOR EACH ROW EXECUTE FUNCTION app_public.node_phrasings_refresh_targets_for_self();
...
CREATE OR REPLACE FUNCTION app_public.node_ratings_refresh_targets_for_self() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
	IF (
		...
		UPDATE app_public."nodeRatings" SET "c_accessPolicyTargets" = array[
...
CREATE TRIGGER node_ratings_refresh_targets_for_self AFTER INSERT OR UPDATE ON app_public."nodeRatings" FOR EACH ROW EXECUTE FUNCTION app_public.node_ratings_refresh_targets_for_self();
...
CREATE OR REPLACE FUNCTION app_public.node_revisions_refresh_targets_for_self() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
	IF (
		...
		UPDATE app_public."nodeRevisions" SET "c_accessPolicyTargets" = array[
			(SELECT concat((SELECT "accessPolicy" FROM "nodes" WHERE id = NEW."node"), ':nodes'))
		] WHERE id = NEW.id;
	END IF;
END $$;
DROP TRIGGER IF EXISTS node_revisions_refresh_targets_for_self on app_public."nodeRevisions";
CREATE TRIGGER node_revisions_refresh_targets_for_self AFTER INSERT OR UPDATE ON app_public."nodeRevisions" FOR EACH ROW EXECUTE FUNCTION app_public.node_revisions_refresh_targets_for_self();
...
CREATE OR REPLACE FUNCTION app_public.node_tags_refresh_targets_for_self() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
	IF (
		...
		UPDATE app_public."nodeTags" SET "c_accessPolicyTargets" = (
			SELECT array_agg(
...
CREATE TRIGGER node_tags_refresh_targets_for_self AFTER INSERT OR UPDATE ON app_public."nodeTags" FOR EACH ROW EXECUTE FUNCTION app_public.node_tags_refresh_targets_for_self();
