...
* 2\) Prepare the postgres-operator to restore the backup, into either a new or the current postgres instance/pod-set:
	* 2.1\) Option 1, into a new postgres instance/pod-set that then gets promoted to master (PGO recommended way):
		...
		* 2.1.3\) You may need to restart [possibly twice, if it gets stuck] the `pgo` resource using the Tilt UI here. (this seems to clear the old instance/pod-set, and create a new one, which then tries to load from the specified data-source)
			* 2.1.3.1\) EDIT: For this most recent restore, this step apparently wasn't necessary; however, I also hit various issues along the process. I really need to make the tiltfile script more robust (perhaps by creating more resource-groups with dependency-links between them, or with `update_settings(max_parallel_updates=1)`), so that the tilt-up process succeeds reliably, even when doing database restores and such. (perhaps the main thing is that I just need a "safe" way to redeploy a resource without breaking the resources that depend on it)
