...
pub async fn body_to_bytes<B: Body>(body: B) -> Result<Bytes, Error>
where
	<B as Body>::Error: std::fmt::Debug,
{
	let body_collected = match body.collect().await {
		Ok(a) => a,
...
pub async fn body_to_str<B: Body>(body: B) -> Result<String, Error>
where
	<B as Body>::Error: std::fmt::Debug,
{
