...
use crate::utils::type_aliases::{FReceiver, FSender, JSONValue};
use anyhow::Error;
...
impl MtxMessage {
	...
	pub fn apply_to_mtx_data(
		self,
		section_lifetimes_ref: &mut RwLockWriteGuard<IndexMap<String, MtxSection>>, //section_lifetimes_ref: &flurry::HashMapRef<String, MtxSection>
	) {
		match self {
...
impl Mtx {
	...
	fn start_new_section(&mut self, name: &str, extra_info: Option<String>, old_section_end_time: f64) {
		let new_section = MtxSection { path: format!("{}/{}", self.path_from_root_mtx, name), start_time: old_section_end_time, extra_info, duration: None };
...
pub async fn package_up_mtx_data_and_send_to_channel(id: Arc<Uuid>, section_lifetimes: Arc<RwLock<IndexMap<String, MtxSection>>>, last_data_as_str: Option<String>) -> Option<String> {
	...
	let _ = msg_sender.send_timeout(MtxGlobalMsg::NotifyMtxDataPossiblyChanged(MtxDataWithExtraInfo { id: mtx_data.id, section_lifetimes: mtx_data.section_lifetimes, data_as_str: data_as_str.clone(), last_data_as_str: last_data_as_str.clone() }), Duration::from_millis(500)); // we pick 500ms so it's certain to complete within the 1s interval of the tokio loop within `Mtx::new`
