...
use async_graphql::extensions::{Extension, ExtensionContext, ExtensionFactory, NextExecute, NextParseQuery, NextPrepareRequest, NextRequest, NextSubscribe};
use async_graphql::futures_util::stream::BoxStream;
use async_graphql::parser::types::{ExecutableDocument, OperationType, Selection};
use async_graphql::{PathSegment, Request, Response, ServerError, ServerResult, Variables};
	...
	for (idx, s) in path.iter().enumerate() {
		...
		match s {
			PathSegment::Index(idx) => {
				let _ = write!(&mut path_str, "{}", idx);
			},
			PathSegment::Field(name) => {
				let _ = write!(&mut path_str, "{}", name);
			},
		}
	}
	if path_str.len() == 0 {
		path_str.push_str("n/a");
	}
	path_str
}
