...
use super::{k8s::cert_handling::get_reqwest_client_with_k8s_certs, type_aliases::JSONValue};
use anyhow::{anyhow, bail, ensure, Context, Error};
use axum::http;
use bytes::Bytes;
use futures::StreamExt;
use http_body_util::{Empty, Full};
use hyper::{body::Body, upgrade};
use hyper_rustls::HttpsConnector;
use itertools::Itertools;
use reqwest::{Body as ReqwestBody, Url};
...
use crate::{
	domains::{get_server_url, DomainsConstants},
	utils::k8s::{
		cert_handling::{get_hyper_client_with_k8s_certs, get_rustls_config_that_accepts_k8s_certs},
		k8s_client::upgrade_to_websocket,
		k8s_structs::K8sSecret,
	},
};
...
pub async fn get_k8s_pod_basic_infos(namespace: &str, filter_to_running_pods: bool) -> Result<Vec<K8sPodBasicInfo>, Error> {
	...
	let req = client.get(format!("https://{k8s_host}:{k8s_port}/api/v1/namespaces/{namespace}/pods{pod_filters_str}")).header("Content-Type", "application/json").header("Authorization", format!("Bearer {token}")).body(json!({}).to_string()).build()?;
	...
	let pod_infos = (|| {
		...
		for pod_info_json in res_as_json.as_object()?.get("items")?.as_array()? {
			...
			pod_infos.push(K8sPodBasicInfo { name: pod_name.to_owned(), creation_time_str: creation_time_str.to_owned() });
		}
		Some(pod_infos)
	})()
...
pub async fn get_or_create_k8s_secret(name: String, namespace: &str, new_data_if_missing: Option<JSONValue>) -> Result<K8sSecret, Error> {
	...
	let req = client.get(format!("https://{k8s_host}:{k8s_port}/api/v1/namespaces/{namespace}/secrets/{name}")).header("Content-Type", "application/json").header("Authorization", format!("Bearer {token}")).body(json!({}).to_string()).build()?;
	let res = client.execute(req).await?;
...
pub async fn exec_command_in_another_pod(pod_namespace: &str, pod_name: &str, container: Option<&str>, command_name: &str, command_args: Vec<String>, allow_utf8_lossy: bool) -> Result<String, Error> {
	...
	let req = hyper::Request::builder()
		.uri(format!("https://kubernetes.default.svc.cluster.local/api/v1/namespaces/{}/pods/{}/exec{}", pod_namespace, pod_name, query_str))
		.method("GET")
		.header("Authorization", format!("Bearer {token}"))
