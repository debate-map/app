...
pub fn get_k8s_certs() -> Result<Vec<CertificateDer<'static>>, Error> {
    ...
    let certs: Vec<CertificateDer> = rustls_pemfile::certs(cert_file_reader).try_collect()
        .map_err(|err| to_anyhow(err).context("Failed to parse a certificate from the k8s certificate file."))?;
