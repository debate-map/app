...
pub async fn get_argument_impact_pseudo_rating(ctx: &AccessorContext<'_>, argument: &Node, premises: &[Node], user_id: &str, use_average_for_missing: bool) -> Result<NodeRating, Error> {
    if premises.len() == 0 { return Err(anyhow!("No premises provided.")); }
...
// sync:js[loosely]
pub async fn get_argument_impact_pseudo_ratings(
    ctx: &AccessorContext<'_>,
    argument: &Node, premises: &Vec<Node>, user_ids: Option<&Vec<String>>, use_average_for_missing: bool,
) -> Result<Vec<NodeRating>, Error> {
	let mut result = vec![];
