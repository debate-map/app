...
use crate::db::node_child_links::{get_node_child_links, get_node_child_link};
...
#[derive(Default)] pub struct MutationShard_DeleteNodeChildLink;
#[Object] impl MutationShard_DeleteNodeChildLink {
	async fn delete_node_child_link(&self, gql_ctx: &async_graphql::Context<'_>, input: DeleteNodeChildLinkInput, only_validate: Option<bool>) -> Result<DeleteNodeChildLinkResult, GQLError> {
		command_boilerplate!(gql_ctx, input, only_validate, delete_node_child_link);
    }
}

#[derive(InputObject, Deserialize)]
pub struct DeleteNodeChildLinkInput {
	pub mapID: Option<String>,
	pub id: String,
}

#[derive(SimpleObject, Debug)]
pub struct DeleteNodeChildLinkResult {
...
#[derive(Default)]
pub struct DeleteNodeChildLinkExtras {
	pub allow_orphaning: bool,
}

pub async fn delete_node_child_link(ctx: &AccessorContext<'_>, actor: &User, input: DeleteNodeChildLinkInput, extras: DeleteNodeChildLinkExtras) -> Result<DeleteNodeChildLinkResult, Error> {
	let DeleteNodeChildLinkInput { mapID, id } = input;
	let result = DeleteNodeChildLinkResult { __: gql_placeholder() };
	...
	let link = get_node_child_link(ctx, &id).await?;
	...
	let child_number_of_parents = get_node_child_links(ctx, None, Some(child_id)).await?.len();
