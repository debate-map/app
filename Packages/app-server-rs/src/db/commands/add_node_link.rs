...
pub async fn assert_new_link_is_valid(ctx: &AccessorContext<'_>, parent_id: &str, new_child_group: ChildGroup, new_child_id: &str, new_child_type: NodeType, permissions: &PermissionGroups) -> Result<(), Error> {
	if !permissions.basic { bail!("You're not signed in, or lack basic permissions."); } 
