...
impl SubscriptionShard_CommandRun {
    async fn commandRuns(&self, ctx: &Context<'_>, id: Option<String>) -> impl Stream<Item = GQLSet_CommandRun<CommandRun>> {
        ...
        let entries: Vec<CommandRun> = rows.into_iter().map(|r| r.into()).collect();
    ...
    async fn commandRun(&self, ctx: &Context<'_>, id: String) -> impl Stream<Item = Option<CommandRun>> {
        let mut wrapper = get_first_item_from_stream_in_result_in_future(self.commandRuns(ctx, Some(id))).await;
