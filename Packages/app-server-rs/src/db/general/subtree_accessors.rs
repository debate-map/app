...
#[async_recursion]
pub async fn populate_subtree_collector(ctx: &AccessorContext<'_>, current_path: String, max_depth: usize, collector: &mut SubtreeCollector) {
    let path_segments: Vec<&str> = current_path.split("/").collect();
    let parent_node_id = path_segments.iter().nth_back(1).map(|a| a.to_string());
    let node_id = path_segments.iter().last().unwrap().to_string();
    //search_info = search_info ?? new GetSubtree_SearchInfo({rootPathSegments: pathSegments});

    let node = get_node(ctx, &node_id).await.unwrap();
    ...
    let node_current = get_node_revision(ctx, &node.c_currentRevision).await.unwrap();
    ...
    if collector.nodes.contains_key(&node.id.0) { return; }
    ...
    if let Some(media_attachment) = node_current.media {
        if !collector.terms.contains_key(media_attachment["id"].as_str().unwrap()) {
            let media = get_media(ctx, &media_attachment["id"].as_str().unwrap().to_owned()).await.unwrap();
...
pub async fn get_db_entry<'a, T: From<Row> + Serialize>(ctx: &AccessorContext<'a>, table_name: &str, filter: &Filter) -> Option<T> {
    let entries = get_db_entries(ctx, table_name, filter).await;
    entries.into_iter().nth(0)
}
pub async fn get_db_entries<'a, T: From<Row> + Serialize>(ctx: &AccessorContext<'a>, table_name: &str, filter: &Filter) -> Vec<T> {
    let query_func = |str1: String| async move {
        ctx.tx.query(&str1, &[]).await
    };
    let (_entries, entries_as_type) = get_entries_in_collection_basic(query_func, table_name, filter).await.unwrap();
    entries_as_type
}
...
pub async fn get_node(ctx: &AccessorContext<'_>, id: &str) -> Option<MapNode> {
    get_db_entry(ctx, "nodes", &Some(json!({
        "id": {"equalTo": id}
    }))).await
}
pub async fn get_node_phrasings(ctx: &AccessorContext<'_>, node_id: &str) -> Vec<MapNodePhrasing> {
    get_db_entries(ctx, "nodePhrasings", &Some(json!({
        "node": {"equalTo": node_id}
    }))).await
}
pub async fn get_terms_attached(ctx: &AccessorContext<'_>, node_rev_id: &str) -> Vec<Term> {
...
pub async fn get_media(ctx: &AccessorContext<'_>, id: &str) -> Option<Media> {
    get_db_entry(ctx, "medias", &Some(json!({
        "id": {"equalTo": id}
    }))).await
}
pub async fn get_node_child_links(ctx: &AccessorContext<'_>, parent_id: Option<&str>, child_id: Option<&str>) -> Vec<NodeChildLink> {
...
pub async fn get_term(ctx: &AccessorContext<'_>, id: &str) -> Option<Term> {
    get_db_entry(ctx, "terms", &Some(json!({
        "id": {"equalTo": id}
    }))).await
}
pub async fn get_node_revision(ctx: &AccessorContext<'_>, id: &str) -> Option<MapNodeRevision> {
