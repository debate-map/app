...
use rust_shared::tokio::time::{self, Instant};
use rust_shared::tokio_postgres::{Client, Row};
use rust_shared::utils::mtx::mtx::Mtx;
use rust_shared::utils::type_aliases::{FSender, JSONValue};
use rust_shared::uuid::Uuid;
use rust_shared::{axum, check_lock_order, futures, new_mtx, tower, tower_http, Lock};
use rust_shared::{serde_json, tokio, utils::time::time_since_epoch_ms, RwLock_Tracked};
use std::borrow::Cow;
...
use crate::utils::db::filter::{entry_matches_filter, FilterOp, QueryFilter};
...
use crate::utils::type_aliases::{ABReceiver, ABSender, DBPoolArc, PGClientObject};
...
impl LQGroupImpl {
	...
	async fn get_lq_instance_or_queue_it_in_progressing_batch(&mut self, lq_key: &LQKey, force_queue_lqi: Option<Arc<LQInstance>>, mtx_p: Option<&Mtx>) -> Result<Option<Arc<LQInstance>>, Error> {
		...
		let (batch_index, batch_generation, prior_lqis_in_batch, _lqi_in_batch, _lqi_just_initialized, _batch_msg_receiver) = 'getter_block: {
			...
			if let Some(entry) = self.meta.lqis_awaiting_population.get(lq_key)
				&& force_queue_lqi.is_none()
			{
			...
			// else, create a new lq-instance for this lq-key, and add it to the buffering-batch
				...
				mtx.section("7:insert lqi and such into lqis_awaiting_population");
				{
					...
					self.meta.lqis_awaiting_population.insert(lq_key.to_owned(), LQIAwaitingPopulationInfo { batch_index, batch_generation: batch_gen, prior_lqis_in_batch, lqi: lqi.clone() });
				}
			...
			for new_lqi in instances_in_batch.into_iter() {
				...
				if let Some(old_lqi) = old_lqi {
					...
					// else, the new-lqi is in fact a new instance/allocation (which means we need to merge the old-lqi into the new-lqi, as smoother alternative to erroring/retrying)
						...
						error!(
							"After batch completed, lq-instance was being committed, but an earlier entry was found; this should never happen. Nonetheless, as defensive programming, attempting to resolve by merging the watchers... {}",
							format!("@batch_i:{} @old_count:{} @new_count:{} @final_count:{} @recent_messages:{}", batch_i, /*batch_generation,*/ old_watchers_count, new_watchers.len() - old_watchers_count, new_watchers.len(), self.get_recent_messages_str())
						);
					}
				}
			}
	...
	pub(super) async fn drop_lq_watcher(&mut self, lq_key: &LQKey, stream_id: Uuid) {
		...
		if new_watcher_count == 0 {
			...
			if let Err(err) = MESSAGE_SENDER_TO_MONITOR_BACKEND
				.0
				.broadcast(Message_ASToMB::LQInstanceUpdated {
