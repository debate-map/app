	...
	pub fn get_sql_for_application(&self, left_container_name: &str, right_container_name: &str) -> Result<SQLFragment, Error> {
		match self {
			...
			LQParam::FilterOpValue(field_name, _, op) => {
				let field_name_fragment = SF::merge(vec![SF::ident(SQLIdent::new(left_container_name.to_owned())?), SF::lit("."), SF::ident(SQLIdent::new(field_name.to_owned())?)]);
				let lq_param_name_fragment = SF::merge(vec![SF::ident(SQLIdent::new(right_container_name.to_owned())?), SF::lit("."), SF::ident(SQLIdent::new(self.name())?)]);
