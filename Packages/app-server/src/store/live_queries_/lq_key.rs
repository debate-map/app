...
use rust_shared::{serde_json::{self, json}, anyhow::ensure, utils::type_aliases::JSONValue, anyhow::Error, itertools::Itertools};
...
impl<'de> Deserialize<'de> for LQKey {
    fn deserialize<D>(deserializer: D) -> Result<LQKey, D::Error> where D: Deserializer<'de> {
...
impl std::fmt::Display for LQKey {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result { std::fmt::Display::fmt(&self._str, f) }
}
impl std::fmt::Debug for LQKey {
    // Is this good? Or should it return debug-string for LQKeyData struct instead?
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result { std::fmt::Debug::fmt(&self._str, f) }
}

impl Eq for LQKey {}
impl PartialEq for LQKey {
    fn eq(&self, other: &LQKey) -> bool { self._str.eq(&other._str) }
}
impl Ord for LQKey {
    fn cmp(&self, other: &LQKey) -> Ordering { self._str.cmp(&other._str) }
}
impl PartialOrd for LQKey {
    fn partial_cmp(&self, other: &LQKey) -> Option<Ordering> { self._str.partial_cmp(&other._str) }
}
impl std::hash::Hash for LQKey {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) { self._str.hash(state); }
}