...
use crate::db::access_policies::{QueryShard_AccessPolicy, SubscriptionShard_AccessPolicy};
use crate::db::command_runs::{QueryShard_CommandRun, SubscriptionShard_CommandRun};
...
use crate::db::commands::delete_node_label::MutationShard_DeleteNodeLabel;
...
use crate::db::global_data::{QueryShard_GlobalData, SubscriptionShard_GlobalData};
use crate::db::map_node_edits::{QueryShard_NodeEdit, SubscriptionShard_NodeEdit};
use crate::db::maps::{QueryShard_Map, SubscriptionShard_Map};
use crate::db::medias::{QueryShard_Media, SubscriptionShard_Media};
use crate::db::node_links::{QueryShard_NodeLink, SubscriptionShard_NodeLink};
use crate::db::node_phrasings::{QueryShard_NodePhrasing, SubscriptionShard_NodePhrasing};
use crate::db::node_ratings::{QueryShard_NodeRating, SubscriptionShard_NodeRating};
...
use async_graphql_axum::{GraphQLBatchRequest, GraphQLProtocol, GraphQLRequest, GraphQLResponse, GraphQLSubscription, GraphQLWebSocket};
...
use rust_shared::{
	async_graphql,
	async_graphql::futures_util::task::{Context, Poll},
};
...
pub struct QueryRoot(
	QueryShard_General, QueryShard_General_Backups, QueryShard_General_Subtree, QueryShard_General_Subtree_Old, QueryShard_General_Search, QueryShard_General_TrustedOperators, QueryShard_NodeLabel,
	// table-specific
...
async fn graphiql() -> impl IntoResponse {
	...
	let app_server_host = if env::var("ENVIRONMENT").unwrap_or("DEV".to_owned()) == "DEV" {
		"localhost:5100"
	} else {
		"debatemap.app"
	};
...
async fn graphql_playground() -> impl IntoResponse {
	response::Html(playground_source(GraphQLPlaygroundConfig::new(GRAPHQL_PATH_EXTERNAL).subscription_endpoint(GRAPHQL_PATH_EXTERNAL)))
}
...
pub async fn handle_gql_query_or_mutation(Extension(_client): Extension<HyperClient>, Extension(schema): Extension<RootSchema>, req: Request<AxumBody>) -> Response<AxumBody> {
	...
	let response = Response::builder().header(CONTENT_TYPE, "application/json").body(response_str.into()).unwrap();
	response
}
		...
		Err(ref errors) => {
			for err in errors {
				...
				if let Some(ref source) = err.source {
					...
					if let Some(ref app_err) = source.downcast_ref::<Error>() {
						// cast to AppError
						error!("Test2:{:?}", app_err);
					}
				}
