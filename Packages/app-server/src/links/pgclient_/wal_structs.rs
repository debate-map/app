...
use rust_shared::tokio_postgres::types::{FromSql, Type};
...
impl ColumnInfo {
	pub fn from_column(col: &Column) -> Self {
		Self { name: col.name().unwrap().to_owned(), data_type: Type::from_oid(col.type_id() as u32).unwrap() }
	}
}
...
fn get_basic<'a, T: for<'de> Deserialize<'de>>(data_tuple: &'a Tuple, column: &ColumnInfo, column_i: usize, val_to_json_val: impl Fn(T) -> Result<JSONValue, Error>) -> Result<JSONValue, Error> {
	...
	match cell_data {
		...
		Text(val_as_bytes) => lds_text_to_json_value_using_pg_data_type(val_as_bytes, column.data_type.clone(), val_to_json_val),
	}
}
fn get_array<'a, T: for<'de> Deserialize<'de>>(data_tuple: &'a Tuple, column: &ColumnInfo, column_i: usize, val_to_json_val: impl Fn(T) -> Result<JSONValue, Error>) -> Result<JSONValue, Error> {
	...
	match cell_data {
		...
		Text(val_as_bytes) => {
			...
			let val_as_vec_of_json_val = val_as_vec_of_string
				.into_iter()
				.map(|item_as_str| {
...
fn lds_text_to_json_value_using_pg_data_type<'a, T: for<'de> Deserialize<'de>>(val_as_bytes: &Bytes, data_type: Type, val_to_json_val: impl Fn(T) -> Result<JSONValue, Error>) -> Result<JSONValue, Error> {
	...
	let val_as_type: T = serde_json::from_slice(val_as_u8_slice).map_err(to_anyhow).with_context(|| format!("@type:{} @val_as_text:{}", data_type.name(), String::from_utf8_lossy(val_as_u8_slice).to_string()))?;
