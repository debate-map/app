...
use crate::{db::{access_policies_::{_access_policy::AccessPolicy, _permission_set::APTable}, _shared::access_policy_target::AccessPolicyTarget, access_policies::get_access_policy}, links::db_live_cache::{get_admin_user_ids_cached, get_access_policy_cached}, utils::db::accessors::AccessorContext};
...
pub(super) fn is_user_creator(user_id: Option<&str>, creator_id: &str) -> bool {
    if let Some(user_id) = user_id && user_id == creator_id { true } else { false }
}
...
pub(super) fn try_does_policy_allow_access(user_id: Option<&str>, policy_id: &str, table: APTable) -> Result<bool, Error> {
    ...
    if policy.permissions.for_table(table).access
        && policy.permission_extends_for_user_and_table(user_id, table).map(|a| a.access.clone()) != Some(false) {
        return Ok(true);
    }
