use rust_shared::{
	...
	itertools::Itertools,
	serde_json,
	utils::{auth::jwt_utils_base::UserJWTData, type_aliases::JSONValue},
};
use serde::Serialize;
...
impl RLSApplier {
	pub fn new(jwt_data: Option<UserJWTData>) -> Self {
		Self { jwt_data, last_result_json: None }
	}
	...
	pub fn filter_next_result_for_collection<T: UsesRLS + Clone + Serialize>(&mut self, next_result: Vec<T>) -> (Vec<T>, bool) {
		...
		if let Some(last_result_json) = &self.last_result_json
			&& &next_result_final_json == last_result_json
		{
