	...
	pub fn get_sql_for_application(&self) -> Result<SQLFragment, Error> {
		...
		for (i, (field_name, field_filter)) in self.field_filters.iter().enumerate() {
			...
			for op in field_filter.filter_ops.iter() {
				parts.push(op.get_sql_for_application(SF::ident(SQLIdent::new(field_name.clone())?), op.get_sql_for_value()?));
			}
		}
		parts.push(SF::lit(")"));
...
impl FilterOp {
	...
	pub fn get_sql_for_value(&self) -> Result<SQLFragment, Error> {
		Ok(match self {
			FilterOp::NotEqualsX(val) => json_value_to_guessed_sql_value_param_fragment(&val)?,
	...
	match json_val {
		...
		JSONValue::Array(data) => {
			...
			if most_matches_for_list > 0 {
				if bool_vals.len() == most_matches_for_list {
					return Ok(SF::value(Box::new(bool_vals)));
				}
				if i64_vals.len() == most_matches_for_list {
					return Ok(SF::value(Box::new(i64_vals)));
				}
				if f64_vals.len() == most_matches_for_list {
					return Ok(SF::value(Box::new(f64_vals)));
				}
				if string_vals.len() == most_matches_for_list {
					return Ok(SF::value(Box::new(string_vals)));
				}
...
pub fn json_vals_to_sql_array_fragment(json_vals: &Vec<JSONValue>) -> Result<SQLFragment, Error> {
	Ok(SF::merge(chain!(SF::lit("array[").once(), json_vals_to_fragments(json_vals)?, SF::lit("]").once(),).collect_vec()))
}
#[rustfmt::skip]
