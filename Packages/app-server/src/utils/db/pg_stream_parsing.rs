...
pub fn parse_postgres_array(array_str: &str, items_are_serialized: bool) -> JSONValue {
	...
	let result_as_json_values = result_as_strings
		.into_iter()
		.map(|item_as_str| {
...
impl LDChange {
	pub fn new_data_as_map(&self) -> Option<RowData> {
		...
		for (i, key) in self.columnnames.as_ref()?.iter().enumerate() {
			...
			let value_final = if self.needs_wal2json_jsonval_fixes.unwrap_or(true) {
				// if unspecified, try to apply fixes
	...
	pub fn get_row_id(&self) -> String {
		let id_from_oldkeys = self.oldkeys.clone().and_then(|a| a.data_as_map().get("id").cloned()).and_then(|a| a.as_str().map(|b| b.to_owned()));
...
fn clone_ldchange_val_0with_type_fixes(value: &JSONValue, typ: &str) -> JSONValue {
	...
	match typ {
		"jsonb" => {
			...
			match value.as_str() {
				Some(val_as_str) => serde_json::from_str(val_as_str).unwrap(),
