...
use dotenv::dotenv;
use rust_shared::{
	domains::{get_env, is_prod},
	flume,
	links::app_server_to_monitor_backend::Message_ASToMB,
	once_cell::sync::OnceCell,
	sentry::{self, ClientInitGuard},
	tokio,
	utils::{
		errors_::backtrace_simplifier::simplify_backtrace_str,
		mtx::mtx::{MtxData, MtxDataWithExtraInfo, MtxGlobalMsg, MTX_GLOBAL_MESSAGE_SENDER_AND_RECEIVER},
...
#[cfg(not(unix))]
pub fn set_up_globals_linux() -> Option<bool> {
	None
}
...
async fn try_send_mtx_data_to_monitor_backend(mtx_data: MtxDataWithExtraInfo) {
	if &Some(mtx_data.data_as_str) != &mtx_data.last_data_as_str {
		let mtx_simple = MtxData { id: mtx_data.id, section_lifetimes: mtx_data.section_lifetimes };
