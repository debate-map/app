...
use rust_shared::utils::{general_::func_types::AsyncFn_Args3, type_aliases::JSONValue};

use crate::{
	db::{node_links::get_node_links, nodes::get_node},
...
pub async fn search_up_from_node_for_node_matching_x(
	...
	x_match_func: impl for<'a> AsyncFn_Args3<Result<bool, Error>, &'a AccessorContext<'a>, &'a str, Option<&'a JSONValue>>,
	...
	while current_layer_heads.len() > 0 {
		...
		for layer_head in &current_layer_heads {
			...
			for parent_id in parent_links.iter().map(|a| a.parent.clone()) {
				if layer_head.path.contains(&parent_id) {
					continue;
				} // parent-id is already part of path; ignore, so we don't cause infinite-loop
				if node_ids_to_ignore.contains(&parent_id) {
					continue;
				}
