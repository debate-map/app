...
pub async fn delete_node(ctx: &AccessorContext<'_>, actor: &User, is_root: bool, input: DeleteNodeInput, extras: DeleteNodeExtras) -> Result<DeleteNodeResult, Error> {
	...
	delete(ctx, actor, &node, &extras, &mapID, is_root, incrementEdits, false).await?;

	Ok(DeleteNodeResult { __: gql_placeholder() })
}

async fn delete(ctx: &AccessorContext<'_>, actor: &User, node: &Node, extras: &DeleteNodeExtras, map_id: &Option<String>, is_root: bool, increment_edits: Option<bool>, for_recursive_comments_delete: bool) -> Result<(), Error> {
	...
	let run = Box::pin(async {
		...
		for comment_child in comment_children_to_delete {
			delete(ctx, actor, &comment_child, extras, map_id, is_root, increment_edits, true).await?;
		}
		...
		ctx.with_rls_disabled(
			|| async {
				...
				ctx.tx.execute(r#"DELETE FROM "subscriptions" WHERE node = $1"#, &[&node_id]).await?;
		...
		increment_edits_if_valid(&ctx, Some(actor), map_id.clone(), is_root, increment_edits).await?;
		Ok(())
	});
	run.await
}
