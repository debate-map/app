...
use crate::db::commands::add_subscription::{
    self, add_or_update_subscription, AddSubscriptionInput, AddSubscriptionInputBuilder,
};
use crate::db::general::permission_helpers::{
    assert_user_can_add_child, assert_user_can_add_phrasing,
};
...
pub async fn add_child_node(
    ...
    extras: AddChildNodeExtras,
) -> Result<AddChildNodeResult, Error> {
    let AddChildNodeInput {
        mapID,
        parentID,
        node: node_,
        revision: revision_,
    ...
    let node_id = new_uuid_v4_as_b64();
        ...
        link_.parent.is_none() && link_.child.is_none(),
        err_should_be_null("[input.link.parent and input.link.child]").to_string()
    );
    ...
    let add_node_result = add_node(ctx, actor, node_, Some(node_id.clone()), revision_).await?;
        ...
        add_node_result.nodeID == node_id,
        "The node-id returned by add_node didn't match the node-id-override supplied to it!"
    );

    let add_node_link_result = add_node_link(
        ctx,
        actor,
        false,
        AddNodeLinkInput { link },
        Default::default(),
    )
    .await?;
    ...
    if user_hiddens.notificationPolicy == "S" {
        let subscription = AddSubscriptionInputBuilder::new(add_node_link_result.id.clone())
        ...
        let subscription = AddSubscriptionInputBuilder::new(parentID.clone())
            .with_add_child_node(true)
            .with_add_child_node(true)
            .with_add_node_link(true)
            .with_add_node_revision(true)
    ...
    if !extras.avoid_recording_command_run {
        record_command_run(
            ctx,
            actor,
            "addChildNode".to_owned(),
            to_json_value_for_borrowed_obj(&input)?,
            to_json_value_for_borrowed_obj(&result)?,
