...
pub async fn delete_argument(ctx: &AccessorContext<'_>, actor: &User, is_root: bool, input: DeleteArgumentInput, _extras: NoExtras) -> Result<DeleteArgumentResult, Error> {
	let DeleteArgumentInput { mapID, argumentID, claimID, deleteClaim, incrementEdits } = input;

	if deleteClaim {
		delete_node(ctx, actor, false, DeleteNodeInput { mapID: None, nodeID: claimID, incrementEdits: Some(false) }, Default::default()).await?;
	} else {
	...
	delete_node(ctx, actor, false, DeleteNodeInput { mapID: None, nodeID: argumentID, incrementEdits: Some(false) }, Default::default()).await?;
