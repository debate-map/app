...
use rust_shared::async_graphql::{Enum, InputObject, SimpleObject, ID};
...
use crate::db::commands::transfer_nodes::{NodeInfoForTransfer, NodeTagCloneType, TransferResult};
use crate::db::general::sign_in_::jwt_utils::{get_user_info_from_gql_ctx, resolve_jwt_to_user_info};
use crate::db::node_links::{get_first_link_under_parent, get_highest_order_key_under_parent, get_node_links, ChildGroup, ClaimForm, NodeLink, NodeLinkInput, Polarity};
...
use crate::db::node_tags::{get_node_tags, NodeTag, NodeTagInput, TagComp_CloneHistory};
use crate::db::nodes::get_node;
use crate::db::nodes_::_node::{ArgumentType, Node, NodeInput};
...
pub async fn transfer_using_clone(ctx: &AccessorContext<'_>, actor: &User, transfer: &NodeInfoForTransfer, prev_transfer_result: Option<&TransferResult>, node_id: &str) -> Result<TransferResult, Error> {
	...
	let new_rev = NodeRevisionInput { node: None, phrasing: rev.phrasing, displayDetails: rev.displayDetails, attachments: rev.attachments };
	...
	let tags_showing_clone_history_for_old_node = tags
		.iter()
		.filter(|tag| {
			if let Some(history) = &tag.cloneHistory
				&& history.cloneChain.last() == Some(&node_id.o())
			{
				true
			} else {
				false
	...
	if tags_showing_clone_history_for_old_node.len() == 0 {
		...
		add_node_tag(&ctx, actor, false, AddNodeTagInput { tag: NodeTagInput { nodes: new_nodes, cloneHistory: Some(new_clone_history), ..Default::default() } }, Default::default()).await?;
	}

	Ok(TransferResult::Clone(TransferResult_Clone { new_node_id }))
}
