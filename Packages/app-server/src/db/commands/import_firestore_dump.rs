...
use rust_shared::db_constants::{GLOBAL_MAP_ID, GLOBAL_ROOT_NODE_ID, SYSTEM_POLICY_PRIVATE_GOVERNED_NAME, SYSTEM_POLICY_PUBLIC_UNGOVERNED_NAME, SYSTEM_USER_EMAIL, SYSTEM_USER_ID};
...
use crate::db::_shared::attachments::{Attachment, EquationAttachment, MediaAttachment, QuoteAttachment, ReferencesAttachment, TermAttachment};
...
use crate::db::node_links::{get_node_links, ChildGroup, ClaimForm, NodeLink, NodeLinkInput, Polarity};
...
use crate::db::node_revisions::{ChildOrdering, NodeRevision};
use crate::db::node_tags::NodeTag;
use crate::db::nodes::get_node;
use crate::db::nodes_::_node::{ArgumentType, Node};
...
use crate::db::user_hiddens::{get_user_hiddens, UserHidden};
use crate::db::users::{get_user, get_users, PermissionGroups, User};
...
use super::_command::{gql_placeholder, insert_db_entry_by_id_for_struct, NoExtras};
...
pub async fn import_firestore_dump(ctx: &AccessorContext<'_>, actor: &User, _is_root: bool, input: ImportFirestoreDumpInput, _extras: NoExtras) -> Result<ImportFirestoreDumpResult, Error> {
	...
	let data_str = fs::read_to_string("@Temp_FirestoreImport.json")?.replace("GLOBAL_MAP_00000000001", "GLOBAL_MAP_00000000002").replace("GLOBAL_ROOT_0000000001", "GLOBAL_ROOT_0000000002");
	...
	for (old_id, val) in collections.try_get("nodes")?.try_as_object()? {
		...
		if let Some(root_node_for_map_val) = &root_node_for_map
			&& !importing_maps.contains_key(root_node_for_map_val)
		{
		...
		let entry = Node {
			...
			createdAt: val.try_get("createdAt")?.try_as_i64()?,
			accessPolicy: default_policy_id.o(),
			r#type: match val.try_get("type")?.try_as_i64()? {
				10 => NodeType::category,
				20 => NodeType::package,
				30 => NodeType::multiChoiceQuestion,
				40 => NodeType::claim,
				50 => NodeType::argument,
	...
	for (old_id, val) in collections.try_get("nodePhrasings")?.try_as_object()? {
		let entry = NodePhrasing {
			...
			r#type: match val.try_get("type")?.try_as_i64()? {
				10 => NodePhrasingType::technical,
				20 => NodePhrasingType::standard,
				_ => bail!("Invalid phrasing type"),
			},
	...
	for (old_id, parent_raw_data) in collections.try_get("nodes")?.try_as_object()? {
		...
		if let Some(children_order) = children_order_vec {
			child_ids_ordered.sort_by_cached_key(|id| children_order.iter().position(|a| a == id).unwrap_or(1000));
		}
	...
	for (old_id, val) in collections.try_get("terms")?.try_as_object()? {
		let entry = Term {
			...
			r#type: match val.try_get("type")?.try_as_i64()? {
				10 => TermType::commonNoun,
				20 => TermType::properNoun,
				30 => TermType::adjective,
				40 => TermType::verb,
				50 => TermType::adverb,
				_ => bail!("Invalid term type"),
			},
