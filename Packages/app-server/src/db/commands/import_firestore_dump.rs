		...
		let entry = NodeRevision {
			...
			attachments: vec![
				val.get("equation").filter(|a| !a.is_null()).map(|data| {
					...
					Attachment { equation: Some(serde_json::to_value(equation).unwrap()), media: None, quote: None, references: None, description: None }
				}),
				val.get("media").filter(|a| !a.is_null()).map(|data| {
					...
					Attachment { equation: None, media: Some(serde_json::to_value(media).unwrap()), quote: None, references: None, description: None }
				}),
				val.get("quote").filter(|a| !a.is_null()).map(|data| {
					...
					Attachment { equation: None, media: None, quote: Some(serde_json::to_value(quote).unwrap()), references: None, description: None }
				}),
				val.get("references").filter(|a| !a.is_null()).map(|data| {
					let references = ReferencesAttachment {
						sourceChains: source_chains_from_old_json_data(data.get("sourceChains")).unwrap(),
					};
					Attachment { equation: None, media: None, quote: None, references: Some(serde_json::to_value(references).unwrap()), description: None }
				}),
