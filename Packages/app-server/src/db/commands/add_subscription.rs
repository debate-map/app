...
use crate::{
	db::{
		commands::_command::{command_boilerplate, insert_db_entry_by_id_for_struct, upsert_db_entry_by_id_for_struct},
...
use rust_shared::{
	...
	serde_json::json,
	utils::{db::uuid::new_uuid_v4_as_b64, general_::extensions::ToOwnedV, time::time_since_epoch_ms_i64},
	GQLError,
};
...
impl AddSubscriptionInputBuilder {
	pub fn new(node: String) -> Self {
		Self { node, addChildNode: None, deleteNode: None, addNodeLink: None, deleteNodeLink: None, addNodeRevision: None, setNodeRating: None }
	}
...
#[derive(Default)]
pub struct MutationShard_AddSubscription;
#[Object]
impl MutationShard_AddSubscription {
...
pub async fn add_or_update_subscription(ctx: &AccessorContext<'_>, actor: &User, _is_root: bool, input: AddSubscriptionInput, _extras: NoExtras) -> Result<AddSubscriptionResult, Error> {
	...
	let existing_subscription = get_db_entry::<Subscription>(
		&ctx,
		"subscriptions",
		&Some(json!({
			"node": {"equalTo": nodeID},
