...
use crate::db::node_links::{get_node_links, NodeLink};
...
pub async fn clone_subtree(gql_ctx: &async_graphql::Context<'_>, payload_raw: JSONValue) -> Result<GenericMutation_Result, Error> {
	...
	for rev_old in subtree.nodeRevisions {
		let rev = NodeRevision { id: get_new_id(&rev_old.id), creator: actor_id(), createdAt: time_since_epoch_ms_i64(), node: get_new_id_str(&rev_old.node), ..rev_old.clone() };
	...
	for tag_old in subtree.nodeTags {
		...
		if let Some(clone_history) = tag.cloneHistory.as_mut() {
			...
			if let Some(last_node_in_history) = last_node_in_history
				&& ids.contains(last_node_in_history)
			{
	...
	for old_node_id in nodes_still_needing_clone_history_tag {
		let tag = NodeTag {
			...
			cloneHistory: Some(TagComp_CloneHistory { cloneChain: vec![old_node_id.clone(), get_new_id_str(&old_node_id)] }),
			labels: None,
