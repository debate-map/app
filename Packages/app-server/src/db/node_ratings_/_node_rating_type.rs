...
pub fn rating_value_is_in_range(value: f64, range: &ValueRange) -> bool {
	...
	if left_side && min_adjusted != 0f64 {
		min_adjusted += 0.001;
	}
	if right_side && max_adjusted != 100f64 {
		max_adjusted -= 0.001;
	}
	...
	ranges
		...
		.map(|(index, range)| {
			...
			return ValueRange {
				min: range.0,
				max: range.1,
				center: if center_fractional.fract() == 0.0 {
					center_fractional
				}
				// if average is int, use that
				else if range.0 < 50f64 {
					center_fractional.floor() + 1f64
				}
				// else, if below 50 (anchor), round up toward it
				else {
					center_fractional.floor()
				}, // else, must be above 50 (anchor), so round down toward it
