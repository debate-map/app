...
impl QueryShard_General_Search {
    async fn search_globally(&self, gql_ctx: &async_graphql::Context<'_>, input: SearchGloballyInput) -> Result<Vec<SearchGloballyResult>, GQLError> {
        let SearchGloballyInput { query, search_limit, search_offset } = input;
        ...
        let rows: Vec<Row> = ctx.tx.query_raw(r#"SELECT * from global_search($1, $2, $3)"#, params(&[
            &query, &search_limit_i32, &search_offset_i32
