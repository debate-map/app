...
export const TermAttachmentsUI = observer_mgl(({enabled, newData, Change}: PhrasingDetailsUI_SharedProps)=>{
...
type TermSearchOrCreateUIProps = {
	name: string,
	enabled: boolean,
	onSelect: (id: string)=>void
}

const TermSearchOrCreateUI = observer_mgl(({name, enabled, onSelect}: TermSearchOrCreateUIProps)=>{
	...
	return (
		<>
			...
			<Row mt={5} style={{
				...
				<Button text="Create new term" enabled={enabled} onClick={()=>{
...
const FoundTermUI = observer_mgl(({term, index, enabled, onSelect}: FoundTermUIProps)=>{
