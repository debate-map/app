...
import {MeID, GetNodeL2, SetMapFeatured, DMap, GetNodeLinks, PERMISSIONS} from "dm_common";
import {GetUpdates, RunInAction} from "web-vcore";
import {SLMode} from "UI/@SL/SL.js";
import {Button_SL} from "UI/@SL/SLButton.js";
import {NN} from "js-vextensions";
import {RunCommand_DeleteMap, RunCommand_UpdateMap} from "Utils/DB/Command.js";
import {zIndexes} from "Utils/UI/ZIndexes.js";
import {MapDetailsUI, MapDetailsUIElem} from "../../MapDetailsUI.js";
...
export const DetailsDropDown = observer_mgl(({map}: {map: DMap})=>{
	const [dataError, setDataError] = useState<string|n>(null);
	const detailsUIRef = useRef<MapDetailsUIElem>(null);
	...
	return (
		<DropDown>
			...
			<DropDownContent style={{zIndex: zIndexes.dropdown, position: "fixed", left: 0, borderRadius: "0 0 5px 0"}}><Column>
				<MapDetailsUI ref={detailsUIRef} baseData={map} phase={creatorOrMod ? "edit" : "view"} onChange={newData=>{
					setDataError(detailsUIRef.current!.getValidationError())
				}}/>
				{creatorOrMod &&
					<Row>
						<Button mt={5} text="Save" enabled={dataError == null} title={dataError} onLeftClick={async()=>{
							const mapUpdates = GetUpdates(map, detailsUIRef.current!.getNewData()).ExcludeKeys("layers", "timelines");
