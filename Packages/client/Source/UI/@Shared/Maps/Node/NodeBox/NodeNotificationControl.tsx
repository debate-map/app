...
export const NodeNotificationControl = (props: NodeNotificationControl_Props)=>{
	const {node, backgroundColor, subscriptionLevel} = props;
	...
	return (
		<Div style={{
			...
			<NotificationLevelButton {...{node, backgroundColor}} oldLevel={subscriptionLevel} targetLevel="none">
			...
			<NotificationLevelButton {...{node, backgroundColor}} oldLevel={subscriptionLevel} targetLevel="partial">
				<svg width="18px" height="18px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<NotificationLevelButton {...{node, backgroundColor}} oldLevel={subscriptionLevel} targetLevel="all">
				<svg width="18px" height="18px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const NotificationLevelButton = (props: {node: NodeL3, backgroundColor: chroma.Color, oldLevel: SubscriptionLevel, targetLevel: SubscriptionLevel, children: React.ReactNode})=>{
	...
	return (
		<button
			...
			onClick={()=>{
				if (active) return;
				RunCommand_AddSubscriptionWithLevel({node: node.id, level: targetLevel});
			}}
		>
			{children}
		</button>
