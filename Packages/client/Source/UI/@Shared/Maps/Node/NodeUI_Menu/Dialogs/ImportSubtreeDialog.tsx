...
import {AddNotificationMessage, InfoButton, RunInAction_Set, UseWindowEventListener} from "web-vcore";
...
type ImportSubtreeUI_State = {
	...
	selectedIRs_nodeAndRev_atImportStart: number,
	serverImportInProgress: boolean,
	serverImport_commandsCompleted: number,
	searchQueryGen: number,
};

export const ImportSubtreeUI = observer_mgl((props: {controller: BoxController} & MI_SharedProps)=>{
	const {map, node, controller} = props;

	const [_, reRender] = useReducer(a=>a+1, 0);
	const [state, setState] = useState<ImportSubtreeUI_State>({
		...
		forJSONDMFS_subtreeData: null,
		forJSONCG_subtreeData: null,
		forCSVSL_subtreeData: null,
		showLeftPanel: true,
		process: false,
		importSelected: false,
		selectedIRs_nodeAndRev_atImportStart: 0,
	...
	const {
		...
		selectedIRs_nodeAndRev_atImportStart, searchQueryGen, serverImportInProgress, leftTab, rightTab, showLeftPanel,
	} = state;

	const stash = useRef<{resources: ImportResource[], resources_parseError: Error|n}>({
		resources: [], resources_parseError: null
	});

	const nodeCreationTimer = useRef(new Timer(MAX_TIMEOUT_DURATION, async()=>{
		if (!nodeCreationTimer.current.Enabled) return;
		try {
			console.log(`Starting next iteration of node-creation-timer. (${nodeCreationTimer.current.callCount_total})`);
			const {map, node: rootNodeForImport} = props;
			...
			if (insertPath_indexOfFirstMissingNode != -1) {
				...
				setTimeout(()=>nodeCreationTimer.current.func(), uiState.autoInsert_interval); // run next iteration soon (1s by default)
				return;
			}
	...
	const setTimerEnabled = (enabled: boolean) => {
		const uiState = store.main.maps.importSubtreeDialog;
		nodeCreationTimer.current.Enabled = enabled;
		if (enabled) {
			setTimeout(()=>nodeCreationTimer.current.func(), uiState.autoInsert_interval); // run first iteration soon (1s by default)
		}
		reRender();
	}

	const triggerSearchesToRerun = () => {
		setState(prev=>({...prev, searchQueryGen: prev.searchQueryGen + 1}));
	}

	const estimateResourceUIHeight = (_index: number, _cache: any)=>{
		return 50;
	};

	const renderResource = (index: number, _key: any)=>{
		const resource = stash.current.resources[index];
		const uiState = store.main.maps.importSubtreeDialog;
		...
		return (
			<ImportResourceUI
				key={index}
				importUnderNode={node}
				searchQueryGen={searchQueryGen}
				onNodeCreated={triggerSearchesToRerun}
		...
		<Column style={{
			...
			<Row style={{flex: 1, minHeight: 0}}>
				...
				<Column style={{width: 500}}>
					<Row>
						<Select displayType="button bar" options={GetEntries(ImportSubtreeUI_LeftTab, "ui")} value={leftTab} onChange={val=>setState(prev=>({...prev, leftTab: val}))}/>
						<Text ml={5}>Source type:</Text>
						<Select ml={5} options={DataExchangeFormat_entries_supportedBySubtreeImporter} value={uiState.sourceType} onChange={val=>RunInAction_Set(()=>uiState.sourceType = val)}/>
					</Row>
					{leftTab == ImportSubtreeUI_LeftTab.source &&
					<>
						<Row>
						...
						<TextArea value={sourceText}
							...
							onChange={newSourceText=>{
								const newState = {sourceText: newSourceText} as ImportSubtreeUI_State;
				...
				<Column style={{width: 800, padding: "0 5px"}}>
					<Row>
						...
						<CheckBox /*ml={5}*/ text="Show left panel" value={showLeftPanel} onChange={val=>setState(prev=>({...prev, showLeftPanel: val}))}/>
						<Row ml="auto">
							<CheckBox text={[`Extract resources`, resources.length > 0 && `(${resources.length})`].filter(a=>a).join(" ")} value={process} onChange={val=>setState(prev=>({...prev, process: val}))}/>
							<CheckBox ml={5} text={`Import selected (${importProgressStr})`} enabled={!serverImportInProgress} value={nodeCreationTimer.current.Enabled} onChange={val=>{
								setTimerEnabled(val);
								setState(prev=>({...prev, selectedIRs_nodeAndRev_atImportStart: selectedIRs_nodeAndRev.length}));
							}}/>
							<Button ml={5}
								text={[
									`Import ALL (server)`,
									serverImportInProgress && ` [${state.serverImport_commandsCompleted}/${resources.length}]`,
								].filter(a=>a).join("")}
								enabled={
									...
									!nodeCreationTimer.current.Enabled && !serverImportInProgress
								}
								onClick={async()=>{
									ShowMessageBox({
										...
										onOK: async()=>{
											...
											try {
												const result = await ImportResourcesOnServer(resources, map.id, node.id, resourcesImported=>{
													setState(prev=>({...prev, serverImport_commandsCompleted: resourcesImported}));
												});
												ShowMessageBox({
											...
											setState(prev=>({...prev, serverImportInProgress: false, serverImport_commandsCompleted: 0}));
										},
									});
								}}/>
						</Row>
					...
					<>
						<Row>
							<CheckBox text="Auto-search by title" value={uiState.autoSearchByTitle} onChange={val=>RunInAction_Set(()=>uiState.autoSearchByTitle = val)}/>
							<CheckBox ml={5} text="Show auto-insert tools" value={uiState.showAutoInsertTools} onChange={val=>RunInAction_Set(()=>uiState.showAutoInsertTools = val)}/>
							{uiState.showAutoInsertTools &&
							<>
								<Text ml={10}>Interval:</Text>
								<Spinner ml={5} value={uiState.autoInsert_interval} onChange={val=>RunInAction_Set(()=>uiState.autoInsert_interval = val)}/>
								<Text ml={3}>ms</Text>
							...
							<PolicyPicker allowClear={true} textForNull="(use original policy id)" value={uiState.accessPolicyOverride} onChange={val=>RunInAction_Set(()=>uiState.accessPolicyOverride = val)}>
						...
						<ScrollView>
							...
							<ReactList type="variable" length={resources.length}
								...
								itemSizeEstimator={estimateResourceUIHeight}/>
						</ScrollView>
					</>}
				</Column>
			</Row>
