import {ArgumentType, AttachmentType, CanConvertFromClaimTypeXToY, ClaimForm, GetAttachmentType_Node, GetNodeLinks, GetNodeDisplayText, GetNodeMirrorChildren, GetParentNodeL3, HasAdminPermissions, DMap, NodeL3, NodeType, MeID, GetLinkAtPath, ReversePolarity, OrderKey, AsNodeL1, PERMISSIONS, GetNode} from "dm_common";
...
import {Assert, GetEntries, ModifyString} from "js-vextensions";
import {SlicePath} from "mobx-graphlink";
import {Button, CheckBox, Column, Pre, Row, Select, Text, TextInput} from "react-vcomponents";
...
export const OthersPanel = observer_mgl((props: Props)=>{
	const {show, map, node, path} = props;
	let [convertToType, setConvertToType] = useState<AttachmentType|n>(null);
...
const AtThisLocation = (props: {node: NodeL3, path: string})=>{
