...
import {GetNodeEffects, GetTimelineSteps, DMap, MeID, OrderKey, Timeline, TimelineStep, TimelineStepEffect, TimelineStepEffect_defaultTransitionPeriod, PERMISSIONS} from "dm_common";
import {DragInfo, MakeDraggable} from "web-vcore";
import {Clone, E, GetEntries, ModifyString, ToJSON, VRect, Vector2, WaitXThenRun} from "js-vextensions";
import {Droppable, DroppableProvided, DroppableStateSnapshot} from "@hello-pangea/dnd";
import {Button, Column, Pre, Row, Select, Text, TextArea} from "react-vcomponents";
...
const _StepEditorUI = observer_mgl((props: StepEditorUIProps)=>{
	const {index, map, timeline, step, nextStep, dragInfo} = props;
	const [state, setState] = useState({
		placeholderRect: null as VRect|n,
	});

	const lastHeaderTabDefaultRef = useRef<StepTab | null>(null);
	const nodeHolderRef = useRef<Row | null>(null);

	const startGeneratingPositionedPlaceholder = ()=>{
		const nodeHolder = nodeHolderRef.current;
		if (nodeHolder == null || !nodeHolder.mounted || nodeHolder.root == null) {
			// call again in a second, once node-holder is initialized
			WaitXThenRun(0, ()=>startGeneratingPositionedPlaceholder());
			return;
		}

		const nodeHolderRect = VRect.FromLTWH(nodeHolder.root.getBoundingClientRect());
		...
		const siblingNodeUIs = (Array.from(nodeHolder.root.childNodes) as HTMLElement[]).filter(a=>a.classList.contains("NodeUI"));
		...
		if (!placeholderRect.Equals(state.placeholderRect)) {
			setState({
	...
	if (timelinesUIState.stepTabDefault != lastHeaderTabDefaultRef.current) {
		WaitXThenRun(0, ()=>setTabOverride(timelinesUIState.stepTabDefault));
	}
	lastHeaderTabDefaultRef.current = timelinesUIState.stepTabDefault;
			...
			<Column /* mt={index == 0 ? 0 : 7} */ {...(dragInfo && dragInfo.provided.draggableProps)}
				...
				<Droppable type="NodeL1" droppableId={ToJSON(new DroppableInfo({type: "TimelineStepNodeRevealList", timelineID: timeline.id, stepID: step.id}))} isDropDisabled={!creatorOrMod}>
					{(provided: DroppableProvided, snapshot: DroppableStateSnapshot)=>{
						...
						return (
							<Column ref={c=>{ nodeHolderRef.current = c; provided.innerRef(c?.root); }} {...provided.droppableProps}
								style={E(
									{
								...
								{dragIsOverDropArea && state.placeholderRect &&
									<div style={{
										...
										position: "absolute", left: 7 /* placeholderRect.x */, top: state.placeholderRect.y, right: 7, height: state.placeholderRect.height,
...
export const StepEditorUI = MakeDraggable(({index, step, draggable}: StepEditorUIProps)=>{
