...
type Props = {
	step: TimelineStep,
	effect: TimelineStepEffect|n,
	effectIndex: number
};

export const StepEffectUI_Menu_Stub = (props: Props & {delayEventHandler?: boolean})=>{
	const {delayEventHandler, ...rest} = props;
...
export const StepEffectUI_Menu = observer_mgl((props: Props)=>{
	const {step, effect, effectIndex} = props;
	...
	return (
		<>
			{effect != null && <VMenuItem text={<span>Cut <span style={{fontSize: 10, opacity: 0.7}}>(for moving effect to another step)</span></span> as any}
				...
				onClick={e=>{
					RunInAction_Set(()=>store.main.timelines.copiedNodeEffectInfo = {stepID: step.id, effectIndex, effectData: Clone(effect), asCut: true});
				}}/>}
			{effect != null && <VMenuItem text={<span>Copy <span style={{fontSize: 10, opacity: 0.7}}>(for cloning effect to another step)</span></span> as any} style={liveSkin.Style_VMenuItem()}
				onClick={e=>{
					RunInAction_Set(()=>store.main.timelines.copiedNodeEffectInfo = {stepID: step.id, effectIndex, effectData: Clone(effect), asCut: false});
				}}/>}
			{copiedEffectInfo &&
				<VMenuItem text={[
					...
					if (copiedEffectInfo.asCut) {
						RunInAction_Set(()=>store.main.timelines.copiedNodeEffectInfo = null);
					}
				}}/>}
