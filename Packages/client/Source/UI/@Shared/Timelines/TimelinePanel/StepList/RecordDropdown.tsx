...
import {GetMap, GetTimelineSteps, GetTimelineStepTimesFromStart, GetTimeline} from "dm_common";
import {Assert, CopyText, SleepAsync, VRect, WaitXThenRun} from "js-vextensions";
import React, {useRef, useState} from "react";
import {AddNotificationMessage, RunInAction_Set, TextPlus, TimeToString} from "web-vcore";
...
export const RecordDropdown = observer_mgl(()=>{
	...
	const captureFrame = (frameNumber: number): Promise<void>=>{
		let resolved = false;
		let rejected = false;
	...
	const startRenderingLoop = async(renderConfig: Render_Config, mapState: MapState)=>{
	...
	const startRecording = async()=>{
		...
		startRenderingLoop(renderConfig, mapState).catch(error=>{
	...
	const forceBrowserRender = ()=>{
		setForcedRerenders((forcedRerenders ?? 0) + 1);
	};
	...
	return (
		<DropDown>
			...
			<DropDownContent style={{zIndex: zIndexes.overNavBarDropdown, right: 0, width: 400}}><Column>
				...
				<Row>
					<CheckBox text="Locked map-ui size:" value={uiState.lockedMapSize} onChange={val=>RunInAction_Set(()=>uiState.lockedMapSize = val)}/>
					<Text ml={5}>X:</Text>
				...
				<Row>
					...
					<TextInput ml={5} value={uiState.renderFolderName} onChange={val=>RunInAction_Set(()=>uiState.renderFolderName = val)}/>
					<Button ml={5} p="5px 10px" text="Now" onClick={()=>RunInAction_Set(()=>uiState.renderFolderName = TimeToString(Date.now(), {fileNameSafe: true}))}/>
				...
				<Row>
					...
					<Spinner ml={5} value={uiState.frameRender_minWait} onChange={val=>RunInAction_Set(()=>uiState.frameRender_minWait = val)}/>
					...
					<Spinner ml={5} value={uiState.frameRender_stabilityWait} onChange={val=>RunInAction_Set(()=>uiState.frameRender_stabilityWait = val)}/>
				</Row>
				<Row>
					<Text>Start recording:</Text>
					<CheckBox ml={5} value={uiState.recording} onChange={val=>(uiState.recording ? stopRecording() : startRecording())}/>
				</Row>
