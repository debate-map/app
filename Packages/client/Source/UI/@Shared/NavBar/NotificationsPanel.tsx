...
export const NotificationsPanel = observer_mgl(()=>{
	const notifRef = useRef<(Notification & { command: CommandRun|n})[]>([]);
	...
	const renderItem = (index: number, key: string | number)=>{
		const notifRefVal= notifRef.current;
		const notification = notifRefVal[index];
		...
		return (
			...
			>
				...
				<CommandRunUI panel={true} key={index} run={notification.command!} index={index} last={index === notifRefVal.length - 1}/>}
			</Column>
		);
	}
		...
		<Div style={{
			...
			{mapState && <Row style={{padding: 5, alignItems: "center", justifyContent: "space-between"}}>
				<Row center style={{padding: "8px 0px"}}>
					<CheckBox ml="auto" text="Paint Mode" value={mapState.subscriptionPaintMode} onChange={val=>RunInAction_Set(()=>{ mapState.subscriptionPaintMode = val; })}/>
				...
				{mapState?.subscriptionPaintMode === true && <Row ml={10}>
					...
					<Select ml={5} displayType="button bar" options={GetEntries(SubscriptionLevel, "ui")} value={uiState.paintMode_notificationLevel} onChange={val=>{
						RunInAction_Set(()=>uiState.paintMode_notificationLevel = val);
					}}/>
				</Row>}
			</Row>}
		</Div>
