...
import {GetNotifications, HasAdminPermissions, Me, MeID} from "dm_common";
import React, {useCallback, useState} from "react";
import {Link, NavBarPanelButton, NotificationsUI} from "web-vcore";
...
export const NavBar = observer_mgl(()=>{
		...
		<nav style={{
			...
			<div style={{display: "flex"}}>
				...
				<div style={{
					...
					boxShadow: liveSkin.NavBarBoxShadow(), clipPath: "inset(0 -150px -150px 0)"
				}}>
...
const NotificationNavBarPanelButton = observer_mgl(()=>{
...
export const NavBarPageButton = observer_mgl<NavBarPageButtonProps>(({page, text, panel, active, style, onClick})=>{
	const [hovered, setHovered] = useState(false);
	const currentPage = store.main.page;
	const isActive = active != null ? active : page == currentPage;
	const finalStyle = E(
		{
	...
	const hoverOrActive = hovered || isActive;
	const handleMouseEnter = useCallback(()=>setHovered(true), []);
	const handleMouseLeave = useCallback(()=>setHovered(false), []);
	return (
		<Link actionFunc={actionFunc} style={finalStyle} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onClick={onClick}>
			{text}
			{hoverOrActive && <div style={{position: "absolute", left: 0, right: 0, bottom: 0, height: 2, background: "rgba(100,255,100,1)"}}/>}
		</Link>
	)
})
