...
export const LinkPreserver = observer_mgl(()=>{
	...
	useSubscription<{linkPreserver: LinkPreserverResult}>(LINK_PRESERVER_SUBSCRIPTION, {
		variables: {input: {updateInterval}},
		onData: info=>{
			const {pageRefreshRequested} = info.data.data?.linkPreserver ?? {pageRefreshRequested: false};
