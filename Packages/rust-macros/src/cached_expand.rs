...
pub fn cached_expand_impl(input: TokenStream) -> TokenStream {
	...
	if only_add_markers {
		...
		return pre_tokens.into_iter().chain(input).chain(post_tokens.into_iter()).collect();
	}
	...
	println!("Writes done! Proofs:[{}, {}]", fs::read_to_string(cache_input_path.clone()).unwrap().len(), fs::read_to_string(cache_output_path.clone()).unwrap().len());
