...
(async()=>{
	...
	// pulumi apparently sets the working-directory to Pkg/deploy/Pulumi (at least at this point in execution), so go up to repo-root to write json file
	fs.writeFileSync("../../../PulumiOutput_Public.json", JSON.stringify({
		registryURL: await GetValue(mainRepoUrl),
